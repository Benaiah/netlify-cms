...
export default class API {
  ...
  getCursor(response) {
    const index = response.headers.get("X-Page");
    const pageSize = response.headers.get("X-Per-Page");
    const pageCount = response.headers.get("X-Total-Pages");
  ...
  traverseCursor(cursor, action) {
    ...
    return fetch(link, { headers: this.requestHeaders() })
      .then(res => {
        ...
        return this.getResponseData(res).then(responseData => ({ files: responseData.data, newCursor }));
      });
  }
