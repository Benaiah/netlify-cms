...
export default class API {
  ...
  updateForkWorkflowBranch = async (contentKey, { metadata: metadataArg } = {}) => {
    ...
    if (status === 'draft' && prMetadata) {
      return this.closePR(prMetadata);
    }

    if (prMetadata) {
      const originPRInfo = await this.request(`${this.originRepoURL}/pulls/${prMetadata.number}`);
      const { state: currentState } = originPRInfo;
      if (currentState === 'closed') {
        return this.openPR(prMetadata);
      }
    }

    const [usernameOfFork] = contentKey.split('/');
    const branchName = this.generateBranchName(contentKey);
    const branchPR = (await this.getPRsForBranchName({
      branchName,
      usernameOfFork,
      repoURL: this.originRepoURL,
    })).find(pr => pr.head.ref === branchName);

    if (branchPR) {
      if (branchPR.state === 'closed' && branchPR.merged === true) {
        // The PR has been merged, remove the editorial workflow
        // information and delete the branch
        await this.storeMetadata(contentKey);
      }

      if (status !== 'draft' && branchPR.state === 'closed') {
        // TODO we should _not_ unconditionally re-open PRs when
        // opening the workflow screen
        await this.openPR(branchPR);
      }
      if (status === 'draft' && branchPR.state === 'open') {
        await this.closePR(branchPR);
      }
      return this.storeMetadata(contentKey, { ...metadata, pr: branchPR });
    }

    if (status !== 'draft') {
      const commitMessage = metadata.commitMessage || 'Automatically generated by Netlify CMS';
      const newPR = await this.createPR(commitMessage, branchName);
      return this.storeMetadata(contentKey, { ...metadata, pr: newPR });
    }
  };
  ...
  async listUnpublishedBranches() {
    ...
    const getUpdatedForkWorkflowBranches = flow([
      map(async branch => {
        ...
        await this.updateForkWorkflowBranch(contentKey);
        return branch;
      }),
      onlySuccessfulPromises,
    ]);
  ...
  async updateUnpublishedEntryStatus(collectionName, slug, status) {
    ...
    const updatedMetadata = { ...metadata, status };
    return this.updateForkWorkflowBranch(contentKey, { metadata: updatedMetadata });
  }
  ...
  async deleteUnpublishedEntry(collectionName, slug) {
    ...
    return (
      this.retrieveMetadata(contentKey)
        .then(metadata => (metadata.pr ? this.closePR(metadata.pr) : Promise.resolve()))
